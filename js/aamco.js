...
$(window).scroll(function(){
	...
	$('.hasAnim2').each(function(){
		if(isScrolledIntoView($(this), 700) && hasPlayed.indexOf($(this).attr('id')) == -1){
			hasPlayed.push($(this).attr('id'));
			goAnim($(this).attr('id'));
		}
	})
...
function goAnim(sec){
	...
	if(sec == 'frame2'){
		...
		TweenMax.to($('#frame2').find('.txtline'), 1, {delay:.65, startAt:{left:'+=-700'}, left:-150, opacity:1, ease:Expo.easeOut})
	}
	...
	if(sec == 'frame3'){
		...
		TweenMax.to($('#frame3').find('.txtline'), 1, {delay:.65, startAt:{left:'+=-300'}, left:300, opacity:1, ease:Expo.easeOut})
	}
	...
	if(sec == 'frame5'){
		TweenMax.to($('#frame5').find('.car'), 1, {bottom:-2, ease:Expo.easeOut})
	...
	if(sec == 'frame6'){
		TweenMax.to($('#frame6').find('.road-mask'), 1, {scaleY:0, ease:Power3.easeInOut})
		
		$('#frame6').find('.sign').each(function(i){
			TweenMax.to($(this), .5, {startAt:{y:80}, delay:.5+(i*.15), y:0, opacity:1, ease:Elastic.easeOut, easeParams:[2,2]})
		})
		
		TweenMax.to($('#frame6').find('.txt>h2'), 1, {startAt:{scaleX:.65, scaleY:.65}, delay:.75+(5*.15), scaleX:1, scaleY:1, opacity:1, ease:Elastic.easeOut, easeParams:[2,4]})
		TweenMax.to($('#frame6').find('.cta-btn'), 1, {startAt:{scaleX:.5, scaleY:.5, opacity:1}, delay:1+(5*.15), scaleX:1, scaleY:1, opacity:1, ease:Elastic.easeOut, easeParams:[2,2]})
	}
...
$('#frame6').find('.cta-btn').mouseenter(function(){
	TweenMax.to($(this), .5, {scaleX:1.1, scaleY:1.1, ease:Expo.easeOut})
})
$('#frame6').find('.cta-btn').mouseleave(function(){
